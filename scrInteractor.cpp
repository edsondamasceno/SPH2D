...
void scrInteractor::Reshape(int width, int height) {
	...
	if (width <= height)
		glOrtho(rec_initial.left, rec_initial.right, rec_initial.top,
				rec_initial.bottom * width / height, near_z + aux_z,
				far_z + aux_z);
	else
		glOrtho(rec_initial.left, rec_initial.right * width / height,
...
//	float amb_light[] =  {0.2f, 0.2f, 0.2f, 1.0f};
	...
	GLfloat posicaoLuz[4] = {this->rec_initial.left, (this->rec_initial.top + 10 * this->rec_initial.Width()), far_z, 0.0f};
	//GLfloat posicaoLuz[4] = {0, 50, 50, 0.0f};
...
void scrInteractor::subDisplay() {
	...
	sprintf(label, "X(%4.3f) Y(%4.3f) Z(%4.3f)", this->vec_translation[0],
			this->vec_translation[1], this->vec_translation[2]);
...
void scrInteractor::Init_Menu() {
	...
	text_color[1] = 0.0f;
	text_color[2] = 0.0f;
	rec_button.setValues(0.16, 0.3, 0.51, 0.99);
	strcpy(label, "X:   (free)");
	...
	text_color[0] = 0.0f;
	text_color[1] = 1.0f;
	rec_button.setValues(0.16, 0.3, 0.01, 0.49);
	strcpy(label, "Y:   (free)");
...
bool scrInteractor::WriteScreenImage() {
