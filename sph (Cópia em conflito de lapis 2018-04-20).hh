...
class SPH {
	...
	inline float pressure(float rho) {
    	float p = params.K * pow((rho/params.rho0 - 1.f), params.gamma);      
    	if (p < 0)
      		p *= params.zeta;
    	return p;
	}

  	// Esta parte do codigo foi criado
  	// function mapping density to pressure 1 modified
	inline float pressure1(float rhe) {
		params.rho0 = 2;
  		float p = params.K * pow((rhe/params.rho0 - 1.f), params.gamma);      
    		if (p < 0)
      			p *= params.zeta;
    	return p;
	}
...
public:
	...
	int screenFilenameNumber;
  	...
  	SPH(): h(0), pkernel(h), vkernel(h) { 
	    ...
	    // sph internals
	    params.rel_smoothing_radius = 2.0;

	    // material parameters
	    params.rho0 = 1;
	    params.K = 500;
	    params.nu = 0.002; // 0.002 is a good value if used alone
	    params.xi = 0.4; // 0.01 is a good value if used alone
  	...
  	void init(float jitter = 0.02, float spacing = 0.02) {
    	...
    	vector<Vector> pes = sample_hex(spacing, jitter, Vector(0.6,0,0.5), Vector(1.1, 0.5, 0));
	    ...
	    sphereRadius = 0.15*pow((3.0/4.0)*m/(params.rho0*M_PI),1.0/3.0);
  ...
  void step() {
    ...
    for (int i = 0; i < n_particles(); ++i) {
      	...
      	//if(i < (maximoParticle/2)) {
	      	...
	      	std::cout << "Densidade " << i << "= "  << particles[i].p << std::endl;
	    //}
	    /*
	    else {
	    	particles[i].rhe = 0;
	      	for (int l = 0; l < neighbors[i].size(); ++l) {
	        	particles[i].rhe += m * pkernel.value(neighbors[i][l].d, neighbors[i][l].d_squared);
	      	}
	      	particles[i].p = pressure(particles[i].rhe);
	    }
      	*/
      	//particles[i].p = pressure(particles[i].rho);
		...
		/*
      	if(i < (maximoParticle/2)) {
		    ...
		    pi.a += params.gravity;
	  		...
	  		//colocar o codigo com o rhe
	  		for (int l = 0; l < neighbors[i].size(); ++l) {
	       		if(neighbors[i][l].idx == i)
	        		continue;
	        	Particle &pj = particles[neighbors[i][l].idx];

	        	// pressure forces
		        pi.a -= m * (pi.p/(pi.rhe*pi.rhe) + pj.p/(pj.rhe*pj.rhe)) * pkernel.gradient(neighbors[i][l]);

		        // viscosity
		        pi.a += params.nu * (pj.v - pi.v) * m/(pi.rhe*pj.rhe) * vkernel.second_derivative(neighbors[i][l].d, neighbors[i][l].d_squared);

		        // artificial viscosity
		        float wj = m/pj.rhe * vkernel.value(neighbors[i][l].d, neighbors[i][l].d_squared);
		        w += wj;
		        pi.v_smoothed += pj.v * wj;
		        //update temperature gradient;
				
				//pi.dt += (m/pj.rho)*(pi.T-pj.T)*pkernel.second_derivative(neighbors[i][j].d, neighbors[i][j].d_squared);
		        p1=((m/pj.rhe)*(4.0*pi.rhe/(pi.rhe+pj.rhe)));
		        p2 = (pi.T-pj.T);
		        p3 = (((pi.x-pj.x)*pkernel.gradient(neighbors[i][l]))/(neighbors[i][l].d_squared- 0.001*h*h));
		        pi.dt+= p1*p2*p3;
	      	}
	     	// std::cout <<  " dT = " << pi.T << std::endl;
		     pi.dt*=params.alpha;
