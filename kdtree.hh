  ...
  void drawKdNode(KdNode const *node, Vector const &bmin, Vector const &bmax, Vector const &x, bool containing = true) const {
    if (node->leaf) {
      ...
      if (intersectsSphere(x, bmin, bmax)) {
        glColor3f(1, 0, 0);
        glVertex3f(bmin.x, bmin.y,0);
        glVertex3f(bmin.x, bmax.y,0);
        
        glVertex3f(bmin.x, bmax.y,0);
        glVertex3f(bmax.x, bmax.y,0);
        
        glVertex3f(bmax.x, bmax.y,0);
        glVertex3f(bmax.x, bmin.y,0);
...
public:
  MyKdTree(int bucketsize = 10): bucket_size(bucketsize), root(NULL) {
  }
