...
bool running = false, drawgrid = false, drawsurface = false, drawhash = false, drawtree = false, saveImageFile=false;
    ...
    else
    {
     glPointSize(5);
      ...
      for (int i = 0; i < sph.n_particles(); ++i) {
        ...
        glColor3f(sph.particle(i).C.R,sph.particle(i).C.G,sph.particle(i).C.B);
        glVertex3f(p.x, p.y,p.z);
      }
