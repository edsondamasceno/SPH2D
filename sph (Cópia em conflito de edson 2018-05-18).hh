...
class SPH {
	...
	inline float pressure0(float rho) {
  ...
  // function mapping density to pressure 1
  inline float pressure1(float rho) {
      float p = params.K * pow((rho/params.rho1 - 1.f), params.gamma);      
      if (p < 0)
          p *= params.zeta;
      return p;
  }
...
public:
    ...
    float sphereRadius1;
  	...
  	SPH(): h(0), pkernel(h), vkernel(h) { 
	    ...
	    // material parameters
	    params.rho0 = 1.0; // Density 1
      params.rho1 = 1000; // Density 2
	    params.K = 500; // 500
	    params.nu = 0.001; // 0.002 is a good value if used alone
	    params.xi = 0.6; // 0.01 is a good value if used alone
      ...
      // Teste 1
      //vector<Vector> pos = sample_hex(spacing, jitter, Vector(0,0.2,0), Vector(0.5, 0.7, 0));
      //vector<Vector> pes = sample_hex(spacing, jitter, Vector(0.55,0.2,0.5), Vector(1.05, 0.7, 0));
    ...
    */
    ...
    for(int i = 0; i < n_particles(); i++) {
      ...
      if(i < maximoParticle) {
        ...
        particles[i].p = pressure1(particles[i].rho);
      }
